...
| Select or verify a Google Cloud, Cloudflare, or n8n external target | [`../access/ROUTING.md`](../access/ROUTING.md) | Load one exact service profile and interface before using external access. |
| Configure or inspect an MCP server contract | [`../mcp-servers/README.md`](../mcp-servers/README.md) | Keep transport, dynamic tools, credentials names, and server safety separate from target selection. |
| Understand Codex or historical Kilo harness mapping | [`../adapters/README.md`](../adapters/README.md) | Keep active mappings thin and canonical bodies under their owning domains. |
