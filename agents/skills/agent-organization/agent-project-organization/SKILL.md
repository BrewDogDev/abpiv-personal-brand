...
Read [capability-and-reasoning-classes.md](references/capability-and-reasoning-classes.md) before classifying, selecting, dispatching, or reviewing a Task. Use its portable classes as behavioral requirements, never as authority grants or runtime identifiers.
...
- The Project coordinator owns requirements and decisions, decomposition and dependencies, authority, Task classification and worker/reviewer selection, control state, worker isolation, integration, review gates, and completion evidence.
- `PROJECT.md`, `PLAN.md`, and `projects/ROUTING.md` are coordinator-owned control state. `PLAN.md` is the sole Task ledger.
- Select from observable complexity, consequence, clarity, isolation, and verification evidence. Split oversized work before selection, use the lowest reliable reasoning class, default independent review to `quick`, and escalate to `rigorous` only for a named material risk.
- Ready Task briefs are self-contained execution contracts; workers do not depend on the coordinator conversation for requirements, decisions, or authority.
- Task implementers cannot delegate, alter control state, broaden authority, or silently change their selected capability or reasoning class.
