...
5. Leave the Project `ready` through this review. Do not pre-set execution state; `executing-tasks` owns the dispatch boundary and asks this coordinator to persist it only after the Task preflight passes.
...
6. If review returns a Critical or Important finding, persist `revisions`; after fixes, return to `review` for the amended result.
7. Inspect the actual `TASK.md`, `REPORT.md`, `REVIEW.md`, commits, diff, checks, scope, and unresolved findings.
8. Persist `complete` only when both review verdicts and acceptance evidence pass. Record its commit range, produced interfaces, deviations, and Minor findings.
...
2. Inspect and consume the explicit final whole-Project review and integrated-verification Task's independent verdict and fresh evidence against the Project completion criteria. Do not commission a duplicate closure review; return that Task to `revisions` if its evidence is stale or insufficient.
...
Report Project status, ledger transitions, Task evidence and commit ranges, scheduling and isolation, deviations and amendments, reuse promotions, integrated review and verification, handoff path, archive state, Git/publication states, and blockers.
