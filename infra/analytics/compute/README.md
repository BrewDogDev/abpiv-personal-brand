...
The project uses pinned Plausible, PostgreSQL 16, ClickHouse, and Nginx image digests. Its Docker backend network is internal, its application egress network is separate, and only Nginx joins a non-masqueraded, inter-container-isolated ingress bridge to publish `127.0.0.1:8000`. Durable PostgreSQL, ClickHouse, and application state live under `/srv/plausible` on the independent `abpiv-plausible-data` disk. Startup, restore, backup, and observation refuse to operate unless that exact UUID is mounted.
