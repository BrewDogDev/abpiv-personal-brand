...
metadata_machine_type="$(curl --fail --silent --show-error --header 'Metadata-Flavor: Google' \
  http://metadata.google.internal/computeMetadata/v1/instance/machine-type)"
case "${metadata_machine_type##*/}" in
  e2-custom-medium-6144) reserved_millicores=1000 ;;
  e2-standard-2) reserved_millicores=2000 ;;
  *) echo "Unexpected shared-host machine type." >&2; exit 1 ;;
esac
...
while [ "$(date +%s)" -lt "$end_epoch" ]; do
  ...
  if [ -n "$previous_total" ]; then
    ...
    capacity_percent="$(((total_delta - idle_delta) * logical_cpus * 100 * 1000 / (total_delta * reserved_millicores)))"
    test "$capacity_percent" -lt 70
  fi
  ...
  for service in plausible postgres clickhouse nginx; do
    read -r restart_count health_status < <(docker inspect \
      --format '{{.RestartCount}} {{if .State.Health}}{{.State.Health.Status}}{{else}}missing{{end}}' \
      "abpiv-plausible-${service}-1")
    test "$restart_count" -eq 0
    test "$health_status" = healthy
  done
  if [ "$(cat /etc/abpiv-n8n/mode 2>/dev/null || printf stopped)" = active ]; then
    ...
    n8n_latency="$(curl --fail --silent --show-error --output /dev/null --write-out '%{time_total}' --max-time 5 \
      --header 'Host: workflows.lobst3rs.com' http://127.0.0.1:8080/healthz/readiness)"
    if awk -v value="$n8n_latency" 'BEGIN { exit !(value > 2) }'; then
      exit 1
    fi
  fi
  latency="$(curl --fail --silent --show-error --output /dev/null --write-out '%{time_total}' --max-time 5 \
...
echo "Plausible runtime observation passed for ${duration_seconds} seconds."
