...
- `n8n-fresh-decommission.yml`: separate two-dispatch review/apply flow for an explicitly accepted fresh-start cutover. It proves Cloud Run is manually disabled at zero, never reads or preserves the intentionally abandoned Cloud SQL or legacy bucket data, and binds the reviewed commit plus exact legacy destruction allowlist before deleting the complete old stack.
