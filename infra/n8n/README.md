...
- `n8n-fresh-decommission.yml`: separate two-dispatch review/apply flow for an explicitly accepted fresh-start cutover. It proves Cloud Run is manually disabled at zero or already absent during a partial-decommission retry, never reads or preserves the intentionally abandoned Cloud SQL or legacy bucket data, and binds the reviewed commit plus exact legacy destruction allowlist. SQL arming is target-only and omitted once the instance is absent, so a partial retry cannot recreate deleted legacy resources. When certificate-map resources survive, the manifest binds a temporary `roles/certificatemanager.owner` grant because the ordinary editor role cannot delete maps or map entries; every manifest and apply unconditionally bind, perform, and verify the exact grant's revocation so a later retry also removes a stale binding. Apply retains the six deletion roles through external resource-absence checks, then removes only those obsolete grants and proves full convergence. A failed or cancelled permission phase strictly restores only missing members of those six grants so a reviewed retry remains possible.
- `n8n-fresh-residual-cleanup.yml`: exact follow-up for the Google-controlled private-service networking residue that can remain after Cloud SQL deletion. Google may retain the producer-side subnet for four days, during which the connection cannot be deleted. The first reviewed phase retains only the connection, its reserved range, and `roles/servicenetworking.networksAdmin` while deleting the other five obsolete roles. A later independently reviewed phase deletes only the connection and range, proves both absent, and only then removes the last role. Both phases are exact-commit/hash-bound, retry forward without recreating old resources or permissions, and recheck the private runtime plus public Access/WAF behavior.
...
If Google rejects removal of the private-services connection because the deleted Cloud SQL producer resource is still in its documented retention window, stop retrying the broad workflow. Use `n8n-fresh-residual-cleanup.yml` with `prune-obsolete-permissions` to remove only the five roles no longer needed. After Google releases the producer resource, use `finalize-private-network`; it removes the connection and range before the last service-networking role. Remove the residual HCL and workflow source only after that apply, external absence checks, and a full no-op plan all succeed.
