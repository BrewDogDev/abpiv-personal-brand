...
IN_FLIGHT_TASK_STATUSES = {"executing", "review", "revisions", "blocked"}
UNASSIGNED_PROJECT_STATUSES = {"discovery", "planning", "replanning"}
TASK_REQUIRED_FILES = {
    "ready": {"TASK.md"},
    "executing": {"TASK.md"},
    "blocked": {"TASK.md"},
...
def validate_current_task_route(
    project_status_value: str | None,
    route_current: str,
    ledger: dict[str, str],
    label: str,
    errors: list[str],
) -> None:
    current_value = clean_cell(route_current)
    current_ids = sorted(
        (
            task_id
            for task_id, status in ledger.items()
            if status in IN_FLIGHT_TASK_STATUSES
        ),
        key=lambda task_id: (int(task_id), task_id),
    )
    if not current_ids:
        current_ids = sorted(
            (
    ...
    expected_current = ", ".join(current_ids)
    all_tasks_complete = bool(ledger) and all(
        status == "complete" for status in ledger.values()
    )

    if current_value in {"Not assigned", "None"}:
        sentinel_is_valid = (
            current_value == "Not assigned"
            and project_status_value in UNASSIGNED_PROJECT_STATUSES
            and not current_ids
        ) or (
            current_value == "None"
            and project_status_value == "closing"
            and all_tasks_complete
        )
        if not sentinel_is_valid:
            errors.append(
                f"{label}: current task sentinel '{current_value}' is not valid "
                f"for Project status {project_status_value or 'unknown'} and Task ledger"
            )
        return

    if not re.fullmatch(r"\d+(?:, \d+)*", current_value):
        errors.append(
            f"{label}: current task route must use normalized ascending "
            f"comma-separated Task IDs: {current_value}"
        )
        return

    route_ids = current_value.split(", ")
    if len(route_ids) != len(set(route_ids)):
        errors.append(
            f"{label}: current task route contains duplicate Task ids: {current_value}"
        )
        return
    if route_ids != sorted(
        route_ids,
    ...
    if expected_current and current_value != expected_current:
        errors.append(
            f"{label}: current task route {current_value} does not match "
            f"Task ledger; expected {expected_current}"
        )
        return
    if expected_current:
        return
    ...
    if project_status_value in UNASSIGNED_PROJECT_STATUSES:
        expected_sentinel = "Not assigned"
    elif project_status_value == "closing" and all_tasks_complete:
        expected_sentinel = "None"
    if expected_sentinel is not None:
        errors.append(
            f"{label}: current task route {current_value} does not match "
            f"Task ledger; expected {expected_sentinel}"
        )
    else:
        errors.append(
            f"{label}: Project status {project_status_value or 'unknown'} has no "
            f"in-flight or ready Task for current task route {current_value}"
        )
...
def validate_task_ledger(
    ...
    *,
    project_status_value: str | None = None,
    route_current: str | None = None,
...
def validate_active_project(
    ...
    return validate_task_ledger(
        ...
        errors,
        task_owners,
        project_status_value=actual_status,
